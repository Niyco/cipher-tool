...
class Frequency(constants.Stage):
    def __init__(self, update_output):
        ...
        self.update_vars = (0, 1)
    
    def setup(self, frame, texts):
        super().setup(frame, texts)
        self.radio_1 = ctk.CTkRadioButton(frame, variable=self.mode_var, value=0, text=texts['radio_1'])
        self.radio_2 = ctk.CTkRadioButton(frame, variable=self.mode_var, value=1, text=texts['radio_2'])
        self.radio_3 = ctk.CTkRadioButton(frame, variable=self.mode_var, value=2, text=texts['radio_3'])
        self.checkbox = ctk.CTkCheckBox(frame, variable=self.alpha_ex_var, text=texts['checkbox'])
        self.textbox = tk.Text(frame, bd=0, bg=constants.theme['color']['entry'][constants.mode],
                               fg=constants.theme['color']['text'][constants.mode], width=40,
    ...
    def input_update(self, var, index, mode):
        if var == str(self.mode_var):
            ...
            self.update_vars = (value, self.update_vars[1])
        else:
            value = self.alpha_ex_var.get()
            self.update_vars = (self.update_vars[0], value)
        self.update_output(self)
        
    @staticmethod
    def update(text, mode, alpha_ex):
...
class IoC(constants.Stage):
    ...
    def update(text):
        if len(text) > 1:
            ...
            ioc = sum([letter_freq * (letter_freq - 1) / length for letter_freq in [text.lower().count(letter) for letter in constants.alphabet]])
        else:
            ioc = 0
    ...
    def update_widgets(self, ioc):
        accuracy = 5
        language_difference = round(constants.language_ioc - ioc, accuracy)
        ...
        formatted = (self.texts['text_ioc'] + ' ' + str(round(ioc, accuracy)) + '\n\n' + self.texts['english_ioc']
                     + ' '+ str(round(constants.language_ioc, accuracy)) + language_difference_str + '\n'
